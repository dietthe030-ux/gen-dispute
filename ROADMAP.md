...
This roadmap reflects the current release deployed on Studionet and evidence reviewed through August 9, 2026. It claims no users, traction, or partnerships. Current-deployment evidence verifies deployment parity, exact-ID order creation, issuer registration, finalized 0%, 50%, and 100% dispute outcomes, fail-closed retry, buyer-confirmed release, and expired recovery.
...
| Public frontend | [gen-dispute.vercel.app](https://gen-dispute.vercel.app) is configured for the current release through its production environment. |
...
| Current live dispute settlements | [Order 0](https://explorer-studio.genlayer.com/tx/0xec8d9abce9a3d72a7556d5f3e9769f875874942b53ba8faae9b4010af8b362a1) finalized at 100% buyer refund; [order 1](https://explorer-studio.genlayer.com/tx/0x8a30903d094b1855ec6730cae7b2c38ed392863289961f97f7b29910a8a0a94f) finalized at a 50/50 split. Both reached majority agreement with successful decisive executions and terminal `PAID_OUT` readback. |
| Current live lifecycle completion | [Order 2](https://explorer-studio.genlayer.com/tx/0x11d5c4c9a7238061c79c9561093d7412d978117c8aa337683e3e9c830caf1a65) proves buyer confirmation; [order 3](https://explorer-studio.genlayer.com/tx/0x5ccab3dba913447431953217f325070763c3f6f6693f48f3ef70aec32526e89d) proves missing-receipt `UNDETERMINED`, fresh issuer receipt, retry, and 0% settlement; [order 4](https://explorer-studio.genlayer.com/tx/0xe1c958d8e04b8107c66b5df5332f23a5371634c664cdfa9275b0b68b48b8ead4) proves expired recovery. |
...
- Current live evidence covers supervised 0%, 50%, and 100% outcomes plus buyer confirmation, missing-receipt retry, and expired recovery. This remains a small demonstration sample rather than a reliability rate.
...
| Source-to-deployment integrity | Release source/address parity is verified at `0x7cFC…48E7`; publication provenance is bound through the exact Git commit, release tag, deployment record, and public asset parity. | Preserve exact source/address alignment for every release. | Commit hash, deployment transaction, Explorer source, and production environment review. |
| Transaction success | Current-deployment creation, receipt registration, 0%/50%/100% settlement, buyer confirmation, and expiry recovery writes finalized with majority agreement and successful decisive executions; this small supervised sample is not a reliability rate. | At least 95% of wallet-approved supervised writes finalize with accepted consensus, excluding user rejection. | Reconcile frontend submissions with Explorer receipts and execution results. |
| End-to-end dispute completion | Current orders `0`–`4` prove all three tiers, normal release, fail-closed retry, and expiry recovery. | At least 10 supervised finalized disputes after lifecycle hardening, including repeated evidence for every supported tier. | Explorer transactions and post-transaction contract-state reconciliation. |
...
### Phase 1: Repeat funded end-to-end coverage

- **Problem:** All three payout tiers now have live evidence, but the current deployment has only one supervised transaction for each demonstrated dispute tier.
- **User value:** Reviewers and users can assess repeatability rather than relying on one successful sample per path.
- **Changes:** Run repeated supervised low-value smoke tests for every fixture-backed payout tier and preserve Explorer evidence.
